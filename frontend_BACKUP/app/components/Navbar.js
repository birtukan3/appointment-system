...
import { 
  ...
  CheckCircle, Megaphone, Settings, HelpCircle, Star
} from 'lucide-react';
...
export default function Navbar() {
  ...
  const getNavigationItems = () => {
    const homeOnlyItem = pathname === '/'
      ? [{ name: 'Home', href: '/', icon: Home, color: 'from-blue-500 to-cyan-500' }]
      : [];

    if (!isAuthenticated) return homeOnlyItem;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            ...
            <button 
              ...
              className="md:hidden p-2 rounded-lg text-gray-700 hover:text-blue-600 hover:bg-gray-100 transition-all duration-300"
            >
          ...
          <div className="border-t border-gray-100 p-4 space-y-2">
            {isAuthenticated ? (
              <>
                <button
                  onClick={() => handleNavigation('/profile')}
                  className="w-full px-4 py-3 text-gray-700 hover:bg-gray-100 rounded-xl font-medium flex items-center justify-center space-x-2 transition-all duration-300"
                >
