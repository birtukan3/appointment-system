...
export default function AppointmentsPage() {
  ...
  // Setup polling for real-time updates (fallback for SSE)
  useEffect(() => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      // Check for new unread count
      const newUnreadCount = notifs.filter(n => !n.read).length;
      if (newUnreadCount > unreadCount) {
        // New notification arrived
        const newNotifs = notifs.filter(n => !n.read && !notifications.find(old => old.id === n.id));
        for (const notif of newNotifs) {
          if (notif.type === 'staff_message' || notif.status === 'approved' || notif.status === 'rejected') {
            showInfo(`📢 ${notif.title || 'Staff Update'}: ${notif.message}`, 5000);
          }
        }
  ...
  const getStaffResponseIcon = (status, comment) => {
    if (!comment || status === "Pending") return null;
    if (status === "Approved") {
      return { icon: <ThumbsUp className="h-4 w-4 text-emerald-600" />, text: "Approved with note", color: "emerald" };
    }
    if (status === "Rejected") {
      return { icon: <MessageSquare className="h-4 w-4 text-rose-600" />, text: "Response from staff", color: "rose" };
    }
    return null;
  };

  const getStatusBadge = (status, datetime, comment) => {
    ...
    switch (status) {
      case "Approved": 
        return { icon: <CheckCircle className="h-3 w-3" />, text: comment ? "Approved • Note Added" : "Approved", color: "bg-emerald-100 text-emerald-700" };
      case "Rejected": 
        return { icon: <XCircle className="h-3 w-3" />, text: comment ? "Rejected • Reason Given" : "Rejected", color: "bg-rose-100 text-rose-700" };
      default: 
        return { icon: <Clock className="h-3 w-3" />, text: "Pending Review", color: "bg-amber-100 text-amber-700 animate-pulse" };
    }
  };
  ...
  const getNotificationIcon = (notification) => {
    if (notification.type === 'staff_message' || notification.staffComment) {
  ...
  const getFilteredNotifications = () => {
    ...
    if (selectedNotificationTab === 'staff') {
      return notifications.filter(n => n.type === 'staff_message' || n.staffComment || n.reason);
    }
    if (selectedNotificationTab === 'status') {
      return notifications.filter(n => n.type === 'status_update' || n.status);
    }
  ...
  const statsCards = [
    ...
    { key: 'pending', label: 'Pending Review', value: stats.pending, icon: <Clock className="h-5 w-5" />, color: 'amber', onClick: () => { setStatusFilter("pending"); setCurrentPage(1); } },
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/10 to-purple-50/10 pt-20 pb-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* Welcome Header with Notifications Bell */}
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="relative p-2.5 bg-white/20 backdrop-blur-sm rounded-xl hover:bg-white/30 transition"
                >
                  {unreadCount > 0 ? (
                    <>
          ...
          {/* Notifications Panel - Staff Messages Section */}
              ...
              <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                <div className="flex items-center justify-between p-4 border-b bg-gradient-to-r from-indigo-50 to-purple-50">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h2 className="font-bold text-slate-800">Staff Communications</h2>
                ...
                {getFilteredNotifications().length === 0 ? (
                  ...
                  <div className="divide-y divide-slate-100 max-h-96 overflow-y-auto">
                    {getFilteredNotifications().map((notif) => (
                      <div
                        key={notif.id}
                        className={`p-4 transition-all duration-300 hover:bg-slate-50 cursor-pointer ${
                          !notif.read ? 'bg-gradient-to-r from-indigo-50/50 to-white' : ''
                        }`}
                            ...
                            <div className="flex items-center justify-between flex-wrap gap-2 mb-1">
                              <div className="flex items-center gap-2">
                                <span className="font-semibold text-slate-800">
                                  {notif.title || (notif.status === 'approved' ? 'Appointment Approved' : notif.status === 'rejected' ? 'Appointment Update' : 'Staff Message')}
                                </span>
                                {!notif.read && (
                              ...
                              <span className="text-xs text-slate-400">
                                {formatDistance(parseISO(notif.createdAt || notif.time || notif.timestamp), new Date(), { addSuffix: true })}
                              </span>
                            </div>
                            <p className="text-slate-700 text-sm mb-2">{notif.message}</p>
                            
                            {/* Staff Comment / Reason Display */}
                            {(notif.comment || notif.reason) && (
                              <div className={`mt-2 p-3 rounded-lg ${
                                ...
                                <div className="flex items-start gap-2">
                                  {notif.status === 'approved' ? (
                                    ...
                                    <MessageSquare className="h-4 w-4 text-indigo-600 mt-0.5" />
                                  )}
                                  <div className="flex-1">
                                    <p className="text-xs font-medium text-slate-600">
                                      {notif.status === 'approved' ? '📝 Staff Approval Note:' : notif.status === 'rejected' ? '📋 Reason for update:' : '💬 Staff Comment:'}
                                    </p>
                                    <p className="text-sm text-slate-700 mt-1 leading-relaxed">{notif.comment || notif.reason}</p>
                                  </div>
                                </div>
                              </div>
                            )}
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {appointments.map((app) => {
                  ...
                  const statusBadge = getStatusBadge(app.status, app.datetime, app.comment);
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className={`px-4 py-2 border-b ${
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-medium text-slate-700">
                              {app.status === "Approved" ? "✨ Staff approved your appointment" : "📝 Staff has responded to your request"}
                            </span>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="p-5">
                        ...
                        {/* Staff Response Section - Enhanced */}
                        {hasStaffComment && (
                            ...
                            <div className="p-3">
                              <div className="flex items-start gap-2">
                                ...
                                <div className="flex-1">
                                  ...
                                  <p className="text-xs text-slate-500 mt-0.5">From: {app.providerName}</p>
                                  <div className="mt-2">
                            ...
                            {hasStaffComment ? (
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs text-indigo-600 font-medium">
                                  {app.status === "Approved" ? "Approved with note" : "Response received"}
                                </span>
                              </div>
                            ) : (
        ...
        {/* Details Modal with Staff Response Display */}
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 overflow-y-auto">
            <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
              ...
              <div className="p-6">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-slate-500 mt-0.5">
                            From: {selectedAppointment.providerName} • {formatDistance(parseISO(selectedAppointment.updatedAt || selectedAppointment.createdAt), new Date(), { addSuffix: true })}
                          </p>
                          <div className="mt-3 p-3 bg-white/50 rounded-lg">
                            <p className="text-slate-700 leading-relaxed">{selectedAppointment.comment}</p>
                          </div>
                          {selectedAppointment.reason && selectedAppointment.status === "Rejected" && (
                ...
                {/* Uploaded Documents */}
                {getAppointmentFiles(selectedAppointment.id).length > 0 && (
                  <div className="mb-4 p-4 bg-slate-50 rounded-xl">
                    <p className="text-sm font-medium text-slate-700 mb-2 flex items-center gap-2">
                      <FileText className="h-4 w-4" /> Uploaded Documents
                    </p>
                    <div className="space-y-2">
                      {getAppointmentFiles(selectedAppointment.id).map(file => (
                ...
                <div className="mb-4">
                  <label className="flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-50 text-indigo-600 rounded-xl cursor-pointer hover:bg-indigo-100 transition w-full">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="flex gap-2 justify-center">
                    {[1, 2, 3, 4, 5].map(star => (
                      <button key={star} onClick={() => setFeedback({ ...feedback, rating: star })} className="focus:outline-none transform hover:scale-110 transition">
