  ...
  imports: [
    ...
    TypeOrmModule.forRootAsync({
      ...
      useFactory: (configService: ConfigService) => ({
        ...
        host: configService.get<string>('DB_HOST', 'localhost') as string,
        port: configService.get<number>('DB_PORT', 5432) as number,
        username: configService.get<string>('DB_USERNAME', 'postgres') as string,
        password: configService.get<string>('DB_PASSWORD', 'SmartOffice@2026!') as string,
        database: configService.get<string>('DB_DATABASE', 'appointment_db') as string,
